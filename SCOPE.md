...
- **Room persistence** - rooms remain active while players are connected (plus a short grace period when empty)
- **Room admin** - the creator is the room admin; admin rights survive a page refresh and transfer to another player if the admin leaves
- **Sound modes (exclusive or, chosen at creation)** - "admin controlled" (the admin picks one sound pack for everyone) or "orchestra" (every player picks their own instrument and is heard with it)
- **Synchronized metronome** - the admin can start the metronome on every player's device; each player can stop their own copy
- **Player roster** - always-visible list of who is in the room: avatar, nickname, current instrument, admin crown, playing indicator
...
### User Features
- **No accounts** - play immediately; no registration or login exists
- **Nicknames** - chosen when joining a room, unique within a room (users are prompted to change a taken nickname), remembered locally for next time
- **Avatars** - a cute animal icon picked before joining (Twemoji set); a random one is assigned if the user does not change it
- **Identity in rooms and chat** - players appear with their nickname and avatar

### Customization
- **Theme customization** - customize background color and note trail colors via color picker
- **Settings persistence** - theme choices persisted to browser localStorage (reset available)
...
- **User identification in chat** - messages labeled with the sender's nickname
...
- **Accounts / user profile pages** - no registration, login or persistent user profiles; identity is a per-session nickname and avatar
...
- **Sheet music display / MusicXML rendering** - not implemented
- **Comprehensive test coverage** - no automated test suite; manual verification required
